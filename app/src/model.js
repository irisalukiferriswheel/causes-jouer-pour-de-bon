...
export function normalize(payload){
 ...
 const causes=payload.causes.filter(c=>c && typeof c.id==='string' && !seen.has(c.id) && seen.add(c.id)).map(c=>({
  ...
  country:text(c.country,100),websiteUrl:safeUrl(c.websiteUrl),imageUrl:safeUrl(c.imageUrl),
  goalAmount:fundingMetrics(c).goal,raisedAmount:fundingMetrics(c).raised,currency:fundingMetrics(c).currency,
  campaignStart:Number.isFinite(Date.parse(c.campaignStart))?c.campaignStart:null,campaignEnd:Number.isFinite(Date.parse(c.campaignEnd))?c.campaignEnd:null,
